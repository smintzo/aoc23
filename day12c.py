...
def get_matches(string, pattern):
    ...
    if len(pattern)==0:
        ...
        for match in re.finditer(left_RE, string):
            ...
            if '#' not in string[:match_pos]:
                for rmatch in get_matches(string[match_pos+pattern[0]+1:], [pattern[-1]]):
                    #print('.' * match_pos + '#' * pattern[0] + '.', rmatch)
                    matches.append('.' * match_pos + '#' * pattern[0] + '.' + rmatch)
            
    else:
        #find the middlest pattern
        middle_pattern_i = int(len(pattern)/2)
        ...
        for match in re.finditer(middle_RE, string):
            ...
            for lmatch in get_matches(string[:match_pos - 1], pattern[:middle_pattern_i]):
                for rmatch in get_matches(string[match_pos + middle_pattern + 1:], pattern[middle_pattern_i + 1:]):
