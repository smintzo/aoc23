...
with open('C:\\Users\\smint\\Dropbox\\steve\\advent_of_code\\2023\\input22.txt') as f:
    ...
    for line in lines:
        ...
        blocks.append({ID: i, UL: lend, UH: hend, DOWN: False, SBY: set(), SUPS: set(), PL: True, GOING: False})
        i += 1
        ...
        for x in range(block[UL][0], block[UH][0]+1):
            for y in range(block[UL][1], block[UH][1]+1):
                if max_heights[(x, y)][H] == z:
                    ...
                    block[DOWN] = True
                    block_by_id(max_heights[(x, y)][SUP])[SUPS].add(block[ID])
...
print(block_by_id(546))
print(block_by_id(441))
print(block_by_id(2))

n_bricks = [b for b in blocks if not b[PL]]

total_falling = 0
for brick in n_bricks:
    for reset_brick in blocks:
        reset_brick[GOING] = False
    brick[GOING] = True
    falling = [brick]
    n_falling = 0
    while falling:
        falling.sort(key = lambda b: b[DH][2])
        lowest = falling[0]

        supers = lowest[SUPS]
        for super in supers:
            super_brick = block_by_id(super)
            if super_brick in falling: continue
            sup_sup_by = super_brick[SBY]
            all_gone = True
            for sup_sub in sup_sup_by:
                if not block_by_id(sup_sub)[GOING]: all_gone = False
            if all_gone:
                super_brick[GOING] = True
                falling.append(super_brick)
                n_falling += 1
        del falling[0]
    print(n_falling, total_falling)
    total_falling += n_falling
